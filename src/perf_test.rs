...
pub fn compare_performance_bma_bta() {
    ...
    let channel_1: Vec<Vec<u64>> = vec![vec![1, 2, 3], vec![3, 2, 1]];
    let channel_2: Vec<Vec<u64>> = vec![vec![1, 2, 3], vec![3, 2, 1]];
...
pub fn compare_performance_bma_bmawp() {
    //Fichier resultat
    let mut output_file_bma =
        File::create("resultats_perf/resultats_bma.txt").expect("Impossible de créer le fichier");
    ...
    let mut output_file_bmawp = OpenOptions::new()
        ...
        .expect("Impossible d'ouvrir le fichier");
    // En tête
    writeln!(output_file_bma, "execution,matrix_size,params,time")
        .expect("Impossible d'écrire dans le fichier");
    writeln!(output_file_bmawp, "execution,matrix_size,params,time")
        .expect("Impossible d'écrire dans le fichier");

    let params_crypto = vec![
    ...
    for params in params_crypto {
        ...
        for (n, m) in matrix_size {
            if params.message_modulus.0 >= n && params.message_modulus.0 >= m {
                ...
                let ct_matrix_with_padding =
                    private_key.encrypt_matrix_with_padding(&mut ctx, &matrix);

                let num_iterations = 100;
                for execution in 0..num_iterations {
                    // Temps d'exécution de la première fonction (BMA)
                    let start_time_bma = Instant::now();
                    let ct_0 = public_key.blind_matrix_access(
                        &ct_matrix,
                        &index_line,
                        &index_column,
                        &ctx,
                    ...
                    let ct_1 = public_key.blind_matrix_access(
                        &ct_matrix_with_padding,
                        &index_line,
                        &index_column,
                        &ctx,
                    );
...
fn encode_tensor_into_matrix(channels: Vec<Vec<Vec<u64>>>) -> Vec<Vec<u64>> {
...
mod tests {
    ...
    pub fn test_generate_matrix() {
        ...
        assert!(matrix
            .iter()
            .all(|row| row.iter().all(|&element| element <= p)));
    }
}
...
pub fn eval_perf() {
    ...
    let mut output_file = OpenOptions::new()
        ...
        .expect("Impossible d'ouvrir le fichier");
    // En tête
    writeln!(output_file, "execution,matrix_size,params,time")
        .expect("Impossible d'écrire dans le fichier");
