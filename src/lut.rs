...
impl NLWE {
    ...
    fn from_plain_digits(ctx: &mut Context, private_key: &PrivateKey, digits: Vec<u64>) -> NLWE {
        Self {
            digits: digits
                .iter()
    ...
    pub fn from_plain(value: u64, n: usize, ctx: &mut Context, private_key: &PrivateKey) -> Self {
        ...
        Self::from_plain_digits(ctx, private_key, digits)
    }
            ...
            nlwes = Array::from_shape_fn(IxDyn(&vec![p as usize; d]), |indices| {
                ...
                NLWE {
                    digits: Vec::from_iter((0..n).map(|i| {
                        public_key.lut_extract(&luts[&IxDyn(&idx)][i], indices[d - 1], ctx)
                    })),
                }
            });
        }
