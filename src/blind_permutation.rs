...
pub fn blind_permutation() {
    ...
    let permutation: Vec<u64> = vec![1, 0, 3, 2]; //---> target = [3,7,1,4,5,2]
    ...
    for perm in permutation.clone() {
        let lwe_permutation = private_key
    ...
    // for lwe in many_lwe{
    //     let mut glwe = GlweCiphertext::new(0_u64,ctx.parameters.glwe_dimension().to_glwe_size(),ctx.parameters.polynomial_size);
    ...
    // result.print(&private_key, &ctx);
    result.print_in_glwe_format(&private_key, &ctx);
    ...
    // let mut ct_32 = LweCiphertext::new(0, ctx.parameters.lwe_dimension.to_lwe_size());
...
fn debug_lwe(
    ...
    let plaintext: Plaintext<u64> = decrypt_lwe_ciphertext(&lwe_sk, lwe);

    let result: u64 = signed_decomposer.closest_representable(plaintext.0) / delta;
...
fn debug_glwe(
    ...
    let decoded: Vec<_> = plaintext_res
        .iter()
        .map(|x| {
            (signed_decomposer.closest_representable(*x.0) / delta).wrapping_neg() % message_modulus
        })
        .collect();
