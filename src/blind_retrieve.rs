...
pub fn blind_retrieve() {
    ...
    let mut ct_16 = LweCiphertext::new(
        0,
        ctx.small_lwe_dimension().to_lwe_size(),
        ctx.ciphertext_modulus(),
    );
    ...
    let lut_retrieve = LUT::from_lwe(&lwe_retrieve, public_key, &ctx);
    let mut lut_sum = LUT(public_key.glwe_sum(&lut_original_array.0, &lut_retrieve.0));
    // rerotate the lut
    ...
    println!(
        "Time depacking the intermediate lut : {:?}",
        duration_depack
    );
    ...
    for original_index in 0..many_lut.len() {
        ...
        let ct_original_index =
            public_key.allocate_and_trivially_encrypt_lwe(original_index as u64, &mut ctx);
